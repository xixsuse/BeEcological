...
public class SearchResultController implements Initializable {
	...
	public void openCenterView(MouseEvent event) {
		...
		try {
			Stage window = (Stage) ((Node)event.getSource()).getScene().getWindow();
		    URL url = new File("src/FXView/CenterView.fxml").toURI().toURL();
		    FXMLLoader loader = new FXMLLoader(url);
			Parent tableViewParent = loader.load();
			Scene tableViewScene = new Scene(tableViewParent);
			window.setScene(tableViewScene);
			...
			CenterViewController controller = (CenterViewController) loader.getController();
			if(UserBean.instance != null) {
				controller.loginGroup.setVisible(false);
				controller.userGroup.setVisible(true);
